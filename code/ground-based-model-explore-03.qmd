---
title: "ground-based-model-explore-03"
...
We will go through several steps to select predictors for a final XGBoost Model. This is the third step, in which we will eliminate predictors from the set of 21 predictors we chose in step 2. In the first step, we used all of the 273 available predictors, and obtained a cross-validation accuracy (mean) of 0.957 with standard error of 0.0055. In the second step, we used 21 predictors and obtained a cross-validation accuracy (mean) of 0.959 with standard error of 0.0063. 

We will now eliminate predictors from the set of 21 predictors to see if we can simplify the model without sacrificing accuracy. Our strategy will be to examine the correlation between predictors and to remove predictors that appear to be redundant. We will use the Variable Importance Plot (VIP) from step 2 to decide which redundant predictors to remove.

We removed 5 contras features in this step, resulting in a model with 16 predictors.
...
```{r}
model_version = "03" # string used for file names
```
...
```{r}
cor_matrix <- pix_train %>% select(hsv_1, rgb_seg_SR12_RR4.5_MD30_hsv_1, hsv_2, 
                                   rgb_seg_SR12_RR4.5_MD70_hsv_2, rgb_seg_SR12_RR7.5_MD50_3,
                                   rgb_contrast_L1_W11, hsl_contrast_L3_W11, yuv_contrast_L1_W11,
                                   rgb_contrast_L3_W11, rgb_contrast_L2_W11, hsv_contrast_L2_W11,
                                   lab_contrast_L1_W11, hsv_contrast_L3_W11, yuv_contrast_L2_W11,
                                   rgb_seg_SR12_RR4.5_MD70_lab_3, rgb_seg_SR12_RR7.5_MD70_1,
                                   rgb_seg_SR3_RR4.5_MD50_hsv_1, rgb_seg_SR6_RR7.5_MD50_hsv_1,
                                   rgb_seg_SR3_RR4.5_MD50_hsv_2, rgb_seg_SR6_RR7.5_MD30_hsv_2,
