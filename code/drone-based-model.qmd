...
```{r}
formula <- as.formula(paste0('grass + dead + sand ~ ', paste0(predictors, collapse =" + ")))
...
dirichlet_fit <- DirichReg(Y ~ 
                     ...
                     ortho_4_sd +
                     ortho_5_sd,
                     data = train_data, model = "alternative")
```
...
# When calculating the RMSE use use the number of quadrants as the denominator
# If we just use the regular RMSE we triple count the quadrants (grass, dead, sand per quadrant)
number_of_quadrants <- nrow(test_data)
...
rmse <- prediction_dirichlet |>
  ...
  summarize(rmse = sqrt(sum(error_squared) / number_of_quadrants )) |>
  pull(rmse)
