...
We will go through several steps to select predictors for a final XGBoost Model. This is the fourth step, in which we will begin eliminating predictors one at a time from the set of 16 predictors we had selected at the end of step 3. In step 3 we had a mean cross-validation accuracy of 0.959 (like step 2) with a standard error of 0.00653.

We will eliminate predictors one at a time using variable importance, starting with yuv_contrast_L2_W11
...
```{r}
model_version = "04" # string used for file names
```
